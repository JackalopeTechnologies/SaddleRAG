...
public sealed class WindsurfWriter : IClientWriter
{
    #region Constants

    private const string Name = "windsurf";
    ...
    public WindsurfWriter(string configPath)
    {
    ...
    public static WindsurfWriter ForCurrentUser()
    {
        ...
        string config = Path.Combine(profile, CodeiumDir, WindsurfDir, ConfigFile);
        return new WindsurfWriter(config);
    }
    ...
    public async Task<StatusResult> GetStatusAsync(CancellationToken ct)
    {
        ...
        if (fileExists)
        {
            try
            {
                ...
                string? url = entry?[ServerUrlKey]?.GetValue<string>();
