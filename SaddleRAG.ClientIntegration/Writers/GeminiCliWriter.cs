...
public sealed class GeminiCliWriter : IClientWriter
{
    #region Constants

    private const string Name = "gemini-cli";
    ...
    public GeminiCliWriter(string configPath)
    {
    ...
    public static GeminiCliWriter ForCurrentUser()
    {
        ...
        string config = Path.Combine(profile, GeminiDir, ConfigFile);
        return new GeminiCliWriter(config);
    }
    ...
    public async Task<StatusResult> GetStatusAsync(CancellationToken ct)
    {
        ...
        if (fileExists)
        {
            try
            {
                ...
                string? url = entry?[HttpUrlKey]?.GetValue<string>();
