...
public sealed class InspectScrapeToolTests
{
    ...
    public async Task SummaryReturnsCountsAndBuckets()
    {
        ...
        await repo.InsertManyAsync(MakeMixedAudit(jobId, count: 100), TestContext.Current.CancellationToken);

        var factory = BuildTestFactory(ctx);

        var json = await InspectScrapeTool.InspectScrape(factory,
                                                         jobId,
                                                         status: null,
                                                         skipReason: null,
                                                         host: null,
