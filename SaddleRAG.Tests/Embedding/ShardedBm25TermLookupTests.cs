...
public sealed class ShardedBm25TermLookupTests
{
    private sealed class FakeBm25ShardRepository : IBm25ShardRepository
    {
        ...
        public Task<Bm25Shard?> GetShardAsync(string libraryId,
                                              string version,
                                              int shardIndex,
        ...
        public Task<IReadOnlyList<Bm25Posting>> LoadPostingsAsync(Bm25Shard shard,
                                                                  string term,
                                                                  CancellationToken ct = default)
        {
        ...
        public Task<IReadOnlyList<Bm25Shard>> GetAllShardsAsync(string libraryId,
                                                                string version,
                                                                CancellationToken ct = default) =>
            Task.FromResult<IReadOnlyList<Bm25Shard>>(mShardsByIndex.Values.ToList());

        public Task ReplaceShardsAsync(string libraryId,
                                       string version,
    ...
    public async Task PreloadFetchesOnlyShardsForSuppliedTerms()
    {
        ...
        var hotTerm = allShards[index: 2].InlineTerms.Keys.First();
    ...
    public async Task PreloadIsIdempotent()
    {
        ...
        var term = allShards[index: 1].InlineTerms.Keys.First();
    ...
    public async Task GetPostingsReturnsLoadedTermAfterPreload()
    {
        ...
        (var term, var expectedPostings) = allShards[index: 3].InlineTerms.First();
        await lookup.PreloadAsync([term], TestContext.Current.CancellationToken);
    ...
    {
        var fake = BuildFakeRepoWithFiveShards(out var _);
        const int ShardCount = 5;
        var lookup = new ShardedBm25TermLookup(fake, "lib", "1.0", ShardCount);
