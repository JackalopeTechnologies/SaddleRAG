...
If conversions are slower than you expect, see [Making Docling faster](SaddleRAG.ClientIntegration/Resources/docling-tuning.md). Docling installs a CPU-only PyTorch by default, so an NVIDIA GPU sits idle until you replace it with a CUDA build; that guide also covers thread and batch settings, and the fact that Tesseract is CPU-only while RapidOCR's torch backend is the GPU-capable OCR path.
...
Contributions are welcome — see [CONTRIBUTING.md](./CONTRIBUTING.md). No contributor agreement is required.
