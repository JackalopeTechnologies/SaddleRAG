...
1. **Do the picked models actually have usable ONNX exports?** **Both confirmed in Phase 1 spike.**
   - **Embedding (nomic-embed-text-v1.5 fp16 ONNX, 273 MB):** loads via `Microsoft.ML.OnnxRuntime` 1.26, produces 768-dim L2-normalized vectors, ~80–120 ms per single-text inference on CPU.
   - **Reranker (mxbai-rerank-base-v1 quantized ONNX, 244 MB):** loads via `SentencePieceTokenizer.Create()` against `spm.model`, produces sensible rankings on the Paris/Berlin/Seine test (`Paris=+2.88 > Seine=-0.43 > Berlin=-4.31` — note mxbai ranks Seine above Berlin because Seine mentions the answer entity Paris, demonstrating answer-aware reranking that ms-marco-L6 doesn't do).
   - **Required workaround:** `SessionOptions.GraphOptimizationLevel = GraphOptimizationLevel.ORT_ENABLE_BASIC`. Default `ORT_ENABLE_ALL` triggers `SimplifiedLayerNormFusion` against `InsertedPrecisionFreeCast_` nodes for both models. Drop to BASIC.
   - **Per-model input set varies:** the nomic ONNX takes 3 inputs (`input_ids`, `attention_mask`, `token_type_ids`); the mxbai ONNX takes only 2 (no `token_type_ids` — DeBERTa-v2 has `type_vocab_size=0` so the input was elided from the graph). The production providers must inspect `session.InputMetadata` and supply only the declared inputs; do not assume all three.
