...
public class PageCrawler
{
    ...
    private bool mSiteNeedsExtension;
                                                ...
                                                CancellationToken ct)
    {
        ...
        try
        {
            ...
            // Retry with .html extension if 404 â€” site may require extensions
            if (response != null && response.Status == HttpNotFound && !mSiteNeedsExtension)
            {
                string retryUrl = url + ".html";
                mLogger.LogDebug("Got 404 for {Url}, retrying with .html: {RetryUrl}", url, retryUrl);
                await page.CloseAsync();
                ...
                {
                    mSiteNeedsExtension = true;
                    fetchUrl = retryUrl;
                    mLogger.LogInformation("Site requires .html extensions â€” switching to extension-preserving mode");
                }
            }
    ...
    private static readonly string[] smContentFrameHints =
            ["content", "main", "topic", "body", "detail", "article"];
    ...
    private static readonly string[] smExtensionsToStrip = [".html", ".htm", ".aspx"];
}
