...
public class CategoryAwareChunker
{
    ...
    /// </summary>
    public IReadOnlyList<DocChunk> Chunk(PageRecord page)
    {
        ...
        var initialChunks = page.Category switch
            {
                ...
                DocCategory.ApiReference => ChunkAsApiReference(page),
                DocCategory.Overview => ChunkByHeadings(page),
                DocCategory.ChangeLog => ChunkByVersionBoundaries(page),
                var _ => ChunkByHeadings(page)
            };

        // Hard cap: anything still larger than the embedder's context window
        // gets split at sentence boundaries. Safety net for the embedder.
    ...
    {
        // Keep entire page as one chunk. If too large, split at function boundaries.
        ...
        {
            // Split large samples at double newlines or function boundaries
    ...
    private IReadOnlyList<DocChunk> ChunkAsApiReference(PageRecord page)
    {
        // Try to identify class/member boundaries.
        // For now, fall back to heading-based chunking with QualifiedName extraction.
        var chunks = ChunkByHeadings(page);

        // Attempt to extract qualified names from chunk content
        var enriched = chunks.Select(c => EnrichWithQualifiedName(c)).ToList();
    ...
    private static DocChunk EnrichWithQualifiedName(DocChunk chunk)
    {
        var qualifiedName = ExtractQualifiedName(chunk.Content);
        var result = qualifiedName != null ? chunk with { QualifiedName = qualifiedName } : chunk;
        return result;
    }

    private static string? ExtractQualifiedName(string content)
    {
        // Look for namespace.Class patterns
        var match = Regex.Match(content, @"(?:class|interface|struct|enum)\s+([\w\.]+)", RegexOptions.IgnoreCase);
        string? result = match.Success ? match.Groups[groupnum: 1].Value : null;
        return result;
    }
    ...
    // nomic-embed-text has 2048 token context.
    // Code has a lower chars-per-token ratio (~2.5) than prose (~4).
    // Use 3 chars/token as a conservative estimate for mixed content.
