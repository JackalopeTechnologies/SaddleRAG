...
public class RechunkService
{
                                                  ...
                                                  CancellationToken ct = default)
    {
        ...
        var newIssueDetails = options.BoundaryAudit
                                  ? ChunkBoundaryAudit.EnumerateIssues(newChunks)
                                                      .Take(BoundaryIssueSampleLimit + 1)
                                                      .ToList()
                                  : [];
        ...
        if (!options.DryRun)
        {
            ...
            catch(Exception ex)
            {
                mLogger.LogWarning(ex,
                                   "Vector index reload failed after rechunk; chunks are persisted but search may be stale until next reload_profile"
                                  );
            }
        }
